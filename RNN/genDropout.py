...
import numpy as np
import os
from utility import RNNGenTexteDropout
...
model = RNNGenTexteDropout(len(ids_from_chars.get_vocabulary()), dim_vect, rnn_units, 0.5)
model.build((1, 100))
model.load_weights("RNN/model_trained/trainingDropout/ckpt_50.ckpt")
...
f = open("RNN/texte_generé/RNN.24.05.Dropout.txt", mode = 'w')
for i in range(10):
    start = time.time()
    states = None
